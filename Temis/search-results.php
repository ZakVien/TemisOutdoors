...
if(isset($_POST["search"])){        //search bar query
    $search = test_input($_POST["search"]);
    $_SESSION["search"] = $search;
    $minPrice = 0;
    $maxPrice = 1000;
    $sql = "select * from products where 
        manufacturer like concat('%', :search, '%') or
        name like concat('%', :search, '%') or
        section like concat('%', :search, '%') or
        category like concat('%', :search, '%') or
        subcategory like concat('%', :search, '%')
        order by manufacturer, name";
    $stmt1 = $db->prepare($sql);
    $stmt1->bindValue(":search", $search);
}else if(isset($_POST["reset"])){   //reset filters button
    $_POST['manufacturers'] = "";
    $minPrice = 0;
    $maxPrice = 1000;
    $sql = "select * from products where 
    ...
    $sql = "select * from products where 
        (manufacturer like concat('%', :search, '%') and price <= :maxPrice and price >= :minPrice) or
        (name like concat('%', :search, '%') and price <= :maxPrice and price >= :minPrice) or
        (section like concat('%', :search, '%') and price <= :maxPrice and price >= :minPrice) or
        (category like concat('%', :search, '%') and price <= :maxPrice and price >= :minPrice) or
        (subcategory like concat('%', :search, '%') and price <= :maxPrice and price >= :minPrice) 
        order by manufacturer, name";
    $stmt1 = $db->prepare($sql);
    $stmt1->bindValue(":search", $search);
...
<head>
    ...
    <title>Search Results | Temis Outdoors - Hobby &amp; Supply</title>
    <meta name='description' content='Our unique search results can find everything under our warehouse roof at the click of a button - FREE shipping and returns on everything'>
    ...
    <div class="container products">
            <div class="container w-20 float-left d-none d-lg-block">
                <div class="filterSidebar">
                    <form id="productFilters" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
                        ...
                        <div class="card-body pb-0 pt-0 productPrice d-flex align-items-end">
                            ...
                            <div class="text-center">
                                <div class="mb-sm-2 mb-lg-2">
                                    <form id="viewform" method="POST" action="<?php echo $p["url"];?>">
                                        <input type="hidden" name="productId" value="<?php echo $p['productId'];?>">
                                        <button name="viewform" class="btn btn-primary w-75">View</button>
                                    </form>
                                </div>
