...
<head>
    ...
    <title>All Products | Temis Outdoors - Hobby &amp; Supply</title>
    <meta name='description' content='View all products sold through Temis. We sell archery supplies, hunting gear, fishing gear, and more! - FREE shipping and returns on everything3'>
        ...
        <div class="container w-20 float-left d-none d-lg-block">
            <div class="filterSidebar">
                <form id="productFilters" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
                    ...
                    <br>
                    <?php
                        $sql_category = "select category from products order by category";
                        ...
                        if($sql_category->execute()){
                            ...
                            foreach($categoryitem as $c){
                                array_push($categoryArray, $c["category"]);
                        ?>
                    <?php
                            }
                            $categoryArray = array_unique($categoryArray);
                            foreach($categoryArray as $uc){?>
                    <a href="archery/<?php echo strtolower($uc) . ".php";?>"><?php echo $uc; ?></a><br>
                    <?php }
                        };
                        ?>
                    <br>
                    ...
                    <select name="manufacturers" class="w-100">
                        ...
                        <?php
                        $sql2 = "select * from products where section = 'archery'";
                        $stmt2 = $db->prepare($sql2);
                        if($stmt2->execute()){
                            $productArray = $stmt2->fetchAll();
                            $stmt2->closeCursor();
                            $manufactureArray = [];
                            $manNameNoSpaces = "";
                    ...
                    <input type="hidden" id="maxPrice" name="maxPrice" value="<?php echo $maxPrice;?>" type="hidden">
                    <input type="hidden" id="maximumValue" value="<?php 
                    if($stmt1->execute()){
