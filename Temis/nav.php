...
if(isset($_SESSION["userLogin"])){
    ...
    if(require_once("connect-db.php")){
        $sqlName = "select firstName from users where userId = :userid";
        $stmtName = $db->prepare($sqlName);
        $stmtName->bindValue(":userid", $_SESSION["userLogin"]);
        
        if($stmtName->execute()){
            $users = $stmtName->fetchAll();
...
<nav class="navbar navbar-expand-lg navbar-light bg-faded">
    ...
    <div class="navbar-collapse flex-md-column collapse" id="navbarCollapse">
        <div id="topRow" class="d-flex w-100">
            ...
            <a href="my-account.php" class="d-none d-lg-block"><i class="accIcon far fa-2x fa-user"></i><span class="navCaption"><?php if(!isset($_SESSION["userLogin"]) || empty($_SESSION["userLogin"])){echo 'Log In';}else{echo 'My Account';} ?></span></a>
